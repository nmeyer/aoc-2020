...
// COUNTING STUFF
var I_COUNT = Map();
function countAppear(ingredients) {
...
function parseInput(input) {
    ...
    for (let line of input) {
        let [ingredients, allergens] = line.split('(');
        ingredients = List(ingredients.trim().split(' '));

        allergens = allergens.trim().substr(9);
        allergens = List(allergens.substr(0, allergens.length - 1).split(', '));
        allergens; //?

        foods = foods.push({ ingredients, allergens });

        // Make caches for ease of stuff later
        SET_I = SET_I.union(ingredients);
        SET_A = SET_A.union(allergens);

        // Keep an ingredient count:
        ingredients.forEach(function (i) {
            I_COUNT = I_COUNT.set(i, (I_COUNT.get(i) || 0) + 1)
        });
    }
...
// Loop through all the foods tail-recursion style because screw AoC
function process(foods, a_to_i) {
    if (foods.size == 0) {
        return a_to_i;
    }
    ...
    for (let a of a_set) {
        ...
        let possible = a_to_i.get(a);
        a_to_i = a_to_i.set(a, possible.intersect(i_set)); //?
    }

    return process(foods.rest(), a_to_i);
}
