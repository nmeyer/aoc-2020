...
// vals and memory = 36 bit values / addresses
let MASK = null,
    MEM = {};

function command(line) {    
    var [op, val] = line.split('=');
    
    if (op === 'mask') {
        MASK = val;
    }
    else if (op.startsWith('mem')) {
        let addr = op.substr(4, op.length - 5);
        for (var a of mutateAddr(MASK, addr)) {
            MEM[a] = val;
        }
    }
}

function swapChar(str, i, ch) {
    let start = str.substr(0, i),
    end = str.substr(i + 1);
    return start + ch + end;
}

function mutateAddr(mask, addr) {

    // Which bits are actually floating?
    var bitsToMutate = [];
    for (var i = 0; i < 36; i++) {
        if (mask[i] === 'X')
        bitsToMutate.push(i);
    ...
    addr = parseInt(addr, 10).toString(2).padStart(36, '0');
    
    // Apply Mask. Spots with X are initially set to 0.
    for (var i = 0; i < 36; i++) {
        if (mask[i] === '1') {
            addr = swapChar(addr, i, '1');
        }
        else if (mask[i] === 'X') {
            addr = swapChar(addr, i, '0');
        }
    }

    // Make it a Set so we ignore duplicate addresses.
    // You know...instead of making an accurate recursive algorithm.
    let expandedAddrs = new Set();

    function expandFrom(addr, bitsToMutate) {
        if (bitsToMutate.length == 0)
            return;

        let bit = bitsToMutate[0];

        let addr0 = swapChar(addr, bit, '0');
        expandedAddrs.add(addr0);
        expandFrom(addr0, _.rest(bitsToMutate));

        let addr1 = swapChar(addr, bit, '1');
        expandedAddrs.add(addr1);
        expandFrom(addr1, _.rest(bitsToMutate));
    }
    expandFrom(addr, bitsToMutate);

    return expandedAddrs;
}
// mutateAddr('000000000000000000000000000000X1001X', 42); //?
...
function solveInput(input) {
    ...
    // Sum of all things in memory.
    let total = 0;
    for (var addr in MEM) {
        total += parseInt(MEM[addr], 10);
    }

    return total;
}

// solveInput(TEST_INPUT); //?
solveInput(FULL_INPUT); //?

// command(TEST_INPUT[0]);
// command(TEST_INPUT[1]);
// command(TEST_INPUT[2]);
// command(TEST_INPUT[3]);