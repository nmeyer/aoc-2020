...
function simplify(math) {
    ...
    while (true) {
        let matches = PARENS.exec(math);
        // console.log(matches);
        if (matches == null) {
            break;
        }

        let phrase = matches[0],
            index = matches.index;
        let result = doMath(phrase);
    
        math = math.slice(0,index)
                + result 
                + math.slice(index + phrase.length);
...
function doMath(phrase) {
    console.log('  doMath: ' + phrase);
    // Chop off the enclosing ()
    phrase = phrase.slice(1).substr(0, phrase.length - 2);

    while (true) {
        let m = MATH.exec(phrase);
        // console.log(m);
        if (m == null) {
            break;
        }
        ...
        phrase = phrase.slice(0, m.index)
            + result
            + phrase.slice(m.index + op.length);
