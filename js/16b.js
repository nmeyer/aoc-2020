...
const FULL_INPUT = {
    ...
    'nearby tickets': [
        [797, 604, 555, 482, 170, 789, 928, 930, 331, 605, 402, 720, 924, 743, 852, 61, 213, 713, 540, 51],
...
var RULES = {}; // rules { field: validation }
var ALL_RULE_RANGES = [];
var TICKET = null; // my ticket
var NEARBY_TICKETS = []; // other passenger tix

var FIELD_TO_INDEX = {
    'departure location': _.range(20),
    'departure station': _.range(20),
    'departure platform': _.range(20),
    'departure track': _.range(20),
    'departure date': _.range(20),
    'departure time': _.range(20),
    'arrival location': _.range(20),
    'arrival station': _.range(20),
    'arrival platform': _.range(20),
    'arrival track': _.range(20),
    'class': _.range(20),
    'duration': _.range(20),
    'price': _.range(20),
    'route': _.range(20),
...
function parseNearbyTickets(input) {
    let nearby = input['nearby tickets'];
...
function parseRules(input) {
    ...
    for (var rule of rules) {
        ...
        let matches = ranges.match(/(\d+)\s*-\s*(\d+)/g);
        let [min, max] = matches.slice(1, 3); //?
        RULES[field] = [];
        for (var range of matches) {
            range = range.split('-');
            let minMax = {
                min: parseInt(range[0], 10),
                max: parseInt(range[1], 10)
            };
            RULES[field].push(minMax);
            ALL_RULE_RANGES.push(minMax);
        }
    }
...
function inAllRanges(val, ranges) {
    let isGood = false;

    for (var range of ranges) {
        if (inRange(val, range)) {
            isGood = true;
            break; // value not bad YET.
        }
    }
...
function validate(ticket) {
    // for each val on ticket, make sure it's in ALL ranges
    // for EVERY rule!
    let badValues = [];

    for (var val of ticket) {
        if (!inAllRanges(val, ALL_RULE_RANGES))
            badValues.push(val);
    }
...
function discardBadTickets() {
    console.log(`NEARBY_TICKETS[start] ${ NEARBY_TICKETS.length }`);
    NEARBY_TICKETS = _.filter(NEARBY_TICKETS, function (ticket) {
        return validate(ticket).length == 0;
    });
    console.log(`NEARBY_TICKETS[end] ${ NEARBY_TICKETS.length }`);
}
...
function findFieldIndex(field) {
    ...
    // Trying out this index on ticket.
    for (var fieldIdx = 0; fieldIdx < 20; fieldIdx++) {
        console.log(`Try ${field} index == ${fieldIdx}...`);

        let fieldIndexFound = true;

        // Try out every fieldIdx until one works for every ticket.
        for (var ticket of NEARBY_TICKETS) {
            ticket; //?
            let val = ticket[fieldIdx];
            val; //?
            if (!inAllRanges(val, rule)) {
                // console.log(`---> No.`);
        ...
        if (fieldIndexFound) {
            // console.log(`---> MAYBE == ${ fieldIdx }`);
            field; //?
            possibleIndexes.push(fieldIdx);
            // return fieldIdx; //?
        }
    }

    console.log(`---> Possible Indexes: ${ possibleIndexes }`);
    return possibleIndexes;
}

// After running find possible indexes:
let OPTIONS = {
    'departure location': [
        1, 2, 8, 9, 10,
        11, 12, 14, 17, 18
    ...
    'arrival location': [9, 11],
    'arrival station': [2, 9, 10, 11, 14],
    'arrival platform': [
        0, 1, 2, 3, 5, 7,
        8, 9, 10, 11, 12, 14,
        15, 16, 17, 18
...
function simplify(options) {
    ...
    while (answers.length < 20) {
        ...
        for (var field in options) {
            field; //?
            let possible = options[field]; //?
            if (possible.length == 1) {
                let answer = possible[0];
                console.log(field + ' == ' + answer);
                answers[answer] = field;

                // Remove the answer as a possibility
                delete options[field];
                for (var otherField in options) {
                    let possible = options[otherField];
                    possible = _.without(possible, answer);
                    options[otherField] = possible; //?
                }
            }
        }
...
console.log(179 * 101 * 107 * 211 * 61 * 103) //?
...
function run(input) {
    ...
    let fieldIndexes = [];
    for (var field in FIELD_TO_INDEX) {
        field; //?
        let possible = findFieldIndex(field);
        fieldIndexes[field] = possible;
    }

    console.log(fieldIndexes);

    let finalAnswer = [];
    _.filter(fieldIndexes, (item) => item.length > 0); //?
}
