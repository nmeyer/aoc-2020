...
11: '(?:42){N} (?:31){N}',
// 11: 42 42 31 31
// 11: 42 42 31 ??? would this match?
// 11: 42{n}31{n} // and n is the same for both
// 11: '(42){N} (31){N}',
// 11: '(?<head>(?<h>42)+)(?<tail>(?<t>31)+)', // head = repeats of h, tail = repeats of t, head.length / h.length == tail.length / t.length
5: '1 14 | 15 1',
19: '14 1 | 14 14',
12: '24 14 | 19 1',
16: '15 1 | 14 14',
...
function run(rules, msgs) {
    // Hard-Code 
    let regexString = compile(rules);
    console.log(`Compiled Rules: ${ regexString }`);
    ...
    for (let msg of msgs) {
        ...
        // Check for different repeat times in rule 11:
        // {1} {2} {3} etc.
        for (let N = 1; N < 10; N++) {
            N; //?
            N_regexString = regexString.replace(/N/g, N); //?
            let rulesRegex = new RegExp(N_regexString);
    ...
    return nValid - reject;
}

// let R = new RegExp('^(?:(?:b(?:a(?:bb|ab)|b(?:a|b)(?:a|b))|a(?:bbb|a(?:bb|a(?:a|b))))b|(?:(?:(?:aa|ab)a|bbb)b|(?:(?:a|b)a|bb)aa)a)+(?:(?:(?:b(?:a(?:bb|ab)|b(?:a|b)(?:a|b))|a(?:bbb|a(?:bb|a(?:a|b))))b|(?:(?:(?:aa|ab)a|bbb)b|(?:(?:a|b)a|bb)aa)a)){1}(?:(?:b(?:b(?:aba|baa)|a(?:b(?:ab|(?:a|b)a)|a(?:ba|ab)))|a(?:b(?:(?:ab|(?:a|b)a)b|(?:(?:a|b)a|bb)a)|a(?:bab|(?:ba|bb)a)))){1}$');
// R.exec('babbbbaabbbbbabbbbbbaabaaabaaa'); //?
