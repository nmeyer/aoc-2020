...
const ops = {
    ...
    R: function (arg) {
        state.heading -= arg;
        if (state.heading < 0)
            state.heading = 360 + state.heading;
    },
    F: function (arg) {
        let dx = DX[state.heading] * arg; //?
        let dy = DY[state.heading] * arg; //?
        state.x += dx;
        state.y += dy;
    }
}
...
function run(input) {
    ...
    while (input.length > 0) {
        ...
        runCommand(op, arg);
        console.log(`${op}(${arg}) -> (${state.x},${state.y}) @ ${state.heading}`);
    }
