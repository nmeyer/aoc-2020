...
function emptyRule(state, r, c) {
    // Row Above
    if (r > 0 && c > 0 && state[r - 1][c - 1] == '#')
        return 'L';
    if (r > 0 && state[r - 1][c] == '#')
        return 'L';
    if (r > 0 && c < state[r].length - 1 && state[r - 1][c + 1] == '#')
        return 'L';
    ...
    // if (state[r][c] == '#')
    //     return 'L';
    if (c < state[r].length - 1 && state[r][c + 1] == '#')
        return 'L';

    // Row Below
    if (r < state.length - 1 && c > 0 && state[r + 1][c - 1] == '#')
        return 'L';
    if (r < state.length - 1 && state[r + 1][c] == '#')
        return 'L';
    if (r < state.length - 1 && c < state[r].length - 1 && state[r + 1][c + 1] == '#')
        return 'L';

    return '#';
}

function occupiedRule(state, r, c) {
    let occupiedCount = 0;
...
function nextState(prevState) {
    var nextState = [];
    for (var row of prevState) {
        nextState.push([]);
    }

    for (var r = 0; r < prevState.length; r++) {
        for (var c = 0; c < prevState[r].length; c++) {

            // Floor never changes.
            if (prevState[r][c] === '.') {
                nextState[r][c] = '.';
            }
            else if (prevState[r][c] === 'L') {
                nextState[r][c] = emptyRule(prevState, r, c);
            }
            else if (prevState[r][c] === '#') {
                nextState[r][c] = occupiedRule(prevState, r, c);
            }
...
// var finalState = simulate(TEST_INPUT);
var finalState = simulate(FULL_INPUT);
finalState; //?
countOccupied(finalState); //?